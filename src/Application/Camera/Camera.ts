...
export default class Camera extends EventEmitter {
    ...
    constructor() {
        ...
        this.keyframes = {
            idle: new IdleKeyframe(this.time),
            monitor: new MonitorKeyframe(this.time),
            loading: new LoadingKeyframe(this.time),
            desk: new DeskKeyframe(this.time),
            orbitControlsStart: new OrbitControlsStart(this.time),
        };
    ...
    setFreeCamListeners() {
        const button = document.getElementById('freecam-toggle');
        if (!button) return;
        ...
        button.addEventListener('click', () => {
            if (!this.freeCam) {
                ...
                button.textContent = 'Exit FreeCam';
            } else {
                // Disable freeCam
                this.freeCam = false;
                this.transition(
                ...
                button.textContent = 'Toggle FreeCam';
            }
        });
    }
